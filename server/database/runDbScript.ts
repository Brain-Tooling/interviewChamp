...
client.connect((err) => {
  ...
  client.query(sqlScript, async (error, results) => {
    if (error) {
      ...
      const questionFilePath = path.join(process.cwd(), 'questions.json');
      ...
      for (const category of data) {
        ...
        try {
          const {
            rows: [categoryRow],
          } = await query(selectCategoryQuery, selectCategoryValues);

          let categoryRowToUpdate = categoryRow;

          // If category exists, use existing id
          if (categoryRowToUpdate) {
            console.log(
              `Category '${category.category}' already exists with id ${categoryRowToUpdate.id}`
            );
          } else {
            ...
            const {
              rows: [newCategoryRow],
            } = await query(insertCategoryQuery, insertCategoryValues);
            console.log(
              `Inserted category '${category.category}' with id ${newCategoryRow.id}`
            );
            categoryRowToUpdate = newCategoryRow;
          }
          ...
          for (const question of category.questions) {
            ...
            const selectQuestionQuery =
              'SELECT id FROM Questions WHERE category_id = $1 AND question_content = $2';
            const selectQuestionValues = [
              categoryRowToUpdate.id,
              question.content,
            ];
            ...
            try {
              ...
              if (questionRow) {
                console.log(
                  `Question '${question.content}' already exists in category '${category.category}'`
                );
              } else {
                // Otherwise, insert new question
                const insertQuestionQuery =
                  'INSERT INTO Questions (category_id, question_content) VALUES ($1, $2)';
                ...
                console.log(
                  `Inserted question '${question.content}' into category '${category.category}'`
                );
              }
            } catch (err) {
